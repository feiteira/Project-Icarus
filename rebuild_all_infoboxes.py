#!/usr/bin/env python3
"""Fix infoboxes: proper Wikipedia two-column table structure."""
...
TRIVIA = {
    ...
    "BRICS":         [("Members","10 nations (2024 expansion)"),("Founded","2009 (BRIC), expanded 2024"),("Currency","None (de-dollarization underway)")],
...
def build_infobox(topic_key, image_file):
    ...
    rows = []
    rows.append('{| class="infobox" style="float:right; margin:0 0 1em 1em; font-size:90%; width:280px; border-collapse:collapse; border:1px solid #aaa; background:#f9f9f9;"')
    rows.append('|+ <big>' + topic_key.replace("_", " ") + '</big>')
    rows.append('|- style="text-align:center;"')
    rows.append('| colspan="2" | [[File:' + image_file + '|280px]]')
    for label, value in trivia:
        rows.append('|- style="background:#f0f0f0;"')
...
def fix_page(wiki_title):
    ...
    scan_start = title_idx + 1 if title_idx >= 0 else 0
    ...
    return save_page(wiki_title, new_content, "Fix infobox: proper Wikipedia two-column table")
...
for page in PAGES:
    ok = fix_page(page)
    print(page + ": " + ("OK" if ok else "FAIL"))
    if ok: fixed += 1
    else: failed += 1

print("\nFixed: " + str(fixed) + ", Failed: " + str(failed))