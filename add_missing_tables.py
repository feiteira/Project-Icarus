...
The table was accidentally removed during previous cleanup passes.
"""
...
def build_infobox(topic_key, image_file):
    """Build clean Wikipedia-style infobox."""
...
def fix_page(wiki_title, topic_key, image_file):
    ...
    # Find where body starts: after title, skip blank lines and the image line
    ...
    # Extract body (keep everything from body_start onwards, but strip leading blank lines)
    body = '\n'.join(lines[body_start:])
    body = body.lstrip('\n')

    # Remove any [[File: remnants from body (duplicate images from old infobox)
    ...
    # Clean excessive blank lines from body
    body = re.sub(r'\n{3,}', '\n\n', body).strip('\n')
    ...
    return save_page(wiki_title, new_content, "Add missing infobox table to article sidebar")
...
for page in list(IMAGES.keys()):
    ...
    if wt is None or wt.startswith('#REDIRECT'):
        print(f'{page}: REDIRECT/FAIL - SKIP')
        skipped += 1
        continue

    has_image = '[[File:' + image in wt
    has_infobox_table = '|+ Quick Facts' in wt
    has_wikitable = '{| class="wikitable"' in wt

    if not has_image:
        print(f'{page}: NO IMAGE - SKIP')
        skipped += 1
        continue

    if has_infobox_table and not has_wikitable:
        print(f'{page}: OK (has correct infobox)')
        skipped += 1
        continue

    # Fix it
    print(f'{page}: NEEDS FIX (image={has_image}, table={has_infobox_table}, wikitable={has_wikitable})')
