...
def fix_page(wiki_title, topic_key, image_file):
    ...
    # Check if already has proper infobox
    if 'class="infobox"' in content and topic_key in content:
        return False

    # Remove any old-style wikitable infobox and duplicate images
    lines = content.split("\n")
    cleaned = []
    skip_mode = False
    for line in lines:
        # Skip wikitable blocks that start with {| class="wikitable"
        ...
        if skip_mode:
            if line.strip() == "|}":
                skip_mode = False
            continue
        # Remove duplicate image files (keep first only)
        if line.strip().startswith("[[File:") and len(cleaned) > 0:
            # Check if we already added the first image
            already_have = any("[[File:" in l for l in cleaned)
            if already_have:
                continue
        cleaned.append(line)
    ...
    # Now build correct infobox and insert at top
    infobox = make_infobox(topic_key, image_file)

    # Find where article content starts (after title)
    lines = content.split("\n")
    insert_idx = 0
    ...
    return save_page(wiki_title, new_content, "Fix infobox: Wikipedia-style with float:right class=infobox")
